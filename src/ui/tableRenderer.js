...
import { extractAccountId, generateDataKey } from "../core/dataProcessor.js";
...
export class TableRenderer {
  ...
  renderHeader(table, headers, layout) {
    ...
    headers.forEach((header) => {
      ...
      // Add sorting capability if enabled
      if (
  ...
  createWritebackCell(td, row, header, editedData, rowIndex, currentPage) {
    ...
    const dataKey = generateDataKey(accountId, header.id);
    const cellData = row[header.id];

    if (header.id === "status") {
      this.createStatusDropdown(td, cellData, editedData, dataKey, accountId);
    } else {
   ...
   */
  createStatusDropdown(td, cellData, editedData, dataKey, accountId) {
    ...
    select.addEventListener("change", (e) => {
      console.log(`Status changed for account ${accountId}:`, e.target.value);

      this.onCellEdit(accountId, "status", e.target.value);
   ...
   */
  createTextInput(td, cellData, editedData, dataKey, accountId, fieldId) {
    ...
    input.addEventListener("change", (e) => {
      console.log(
        `${fieldId} changed for account ${accountId}:`,
        e.target.value
      );
      this.onCellEdit(accountId, fieldId, e.target.value);
    });
  ...
  createDataCell(td, cellData, header, rowIndex, layout, tr) {
    ...
    if (header.id === SPECIAL_COLUMNS.PROBABILITY_OF_CHURN) {
  ...
  /**
   * Create churn probability progress bar
   * @param {HTMLElement} td - Cell element
   * @param {Object} cellData - Cell data
   */
  createChurnProgressBar(td, cellData) {
    ...
    if (numValue >= CHURN_RISK_LEVELS.HIGH.threshold) {
      progressBar.classList.add(CHURN_RISK_LEVELS.HIGH.className);
    } else if (numValue >= CHURN_RISK_LEVELS.MEDIUM.threshold) {
      progressBar.classList.add(CHURN_RISK_LEVELS.MEDIUM.className);
    } else if (numValue > CHURN_RISK_LEVELS.LOW.threshold) {
      progressBar.classList.add(CHURN_RISK_LEVELS.LOW.className);
    } else {
      progressBar.classList.add(CHURN_RISK_LEVELS.VERY_LOW.className);
    }
